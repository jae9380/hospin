...
## [HosPin](https://www.hospin.site)

> 🖥️ [Grafana 바로가기](https://grafana.hospin.site)
> 📝 [리팩토링 노트 바로가기](./RefactoringNote.md)
...
| **데이터 출처** | 건강보험심사평가원 제공하는 API, csv                                           |

> ### 배경
>
> 의료비는 생활비에서 적지 않은 비중을 차지한다.
> 하지만 환자가 충분한 의료 정보를 갖지 못한 상태에서 병원을 선택할 경우,
> 적절하지 않은 진단이나 치료로 인해 추가적인 의료비가 발생할 수 있다.
>
> 특히 지방의 경우 의료 인프라와 정보 접근성이 부족하여,
> 환자들이 객관적인 정보보다는 주변 지인의 정보에 의존하는 경우가 많다.
> 이로 인해 자신의 상황에 맞지 않는 병원을 선택하거나,
> 타 지역으로 이동해야 하는 불편함이 발생한다.
>
> HosPin은 건강보험심사평가원에서 제공하는 공공 데이터를 기반으로
> 병원의 평가 등급과 위치 정보를 결합하여 사용자에게 보다 신뢰도 높은 의료 정보를 제공한다.
> 이를 통해 사용자가 단순 거리 기준이 아닌,
> 의료 품질을 고려한 합리적인 병원 선택을 할 수 있도록 돕는 것을 목표로 한다.
