...
const toGenderString = (c: number): GenderStr => (c === 0 ? 'MALE' : c === 1 ? 'FEMALE' : 'UNDEFINED');
...
export class Au {
  ...
  public member = (() => {
    const id = this._id, identifier = this._identifier, name = this._name;
    const phone = this._phone, email = this._email, birth = this._birth, genderCode = this._genderCode;
    return {
      get id() { return get(id); },               set id(v: number) { id.set(v); },
      get identifier() { return get(identifier); },set identifier(v: string) { identifier.set(v); },
      get name() { return get(name); },           set name(v: string) { name.set(v); },
      get phoneNumber() { return get(phone); },   set phoneNumber(v: string) { phone.set(v); },
      get email() { return get(email); },         set email(v: string) { email.set(v); },
      get birth() { return get(birth); },         set birth(v: string) { birth.set(v); },
      get gender() { return toGenderString(get(genderCode)); },
      set gender(v: GenderStr) { genderCode.set(toGenderCode(v)); }
    };
  })();
  ...
  public isLogin() { return this.member.id !== 0; }
  public isLogout() { return !this.isLogin(); }

  public goTo(url: string) { if (browser) goto(url); }
  public replace(url: string) { if (browser) goto(url, { replaceState: true }); }

  public msgInfo(msg: string) { if (browser) window.alert(msg); }
  public msgError(msg: string) { if (browser) window.alert(msg); }
